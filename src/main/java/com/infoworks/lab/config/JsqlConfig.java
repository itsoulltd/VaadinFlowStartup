...
import static com.infoworks.lab.config.DataSourceKeyContainer.Keys;
...
public class JsqlConfig {
    ...
    public SQLExecutor create(String key, DataSourceKeyContainer container) {
        Connection connection = pullConnection(key, container);
        return new SQLExecutor(connection);
    }

    public SQLExecutor create(String key) {
        DataSourceKeyContainer container = getDataSourceKeyContainer();
        return create(key, container);
    }
